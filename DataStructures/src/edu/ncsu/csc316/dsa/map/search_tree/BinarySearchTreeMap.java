...
public class BinarySearchTreeMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
     ...
     * @param compare Type of Comparator to use
     */
    ...
    protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
        ...
        public void rotate(Position<Entry<K, V>> p) {
        	...
        	BSTNode<Entry<K, V>> parentNode = (BSTNode<Entry<K, V>>) parent(p);
        	BSTNode<Entry<K, V>> grandparentNode = (BSTNode<Entry<K, V>>) parent(parent(p));
        	...
        	if (grandparentNode == null) {
        		...
        		if (left(grandparentNode) != null &&
        		    left(grandparentNode).equals(parentNode))
        			
        			relink(grandparentNode, node, true);
        		else
        			relink(grandparentNode, node, false);
        	// Regardless of whether a grandparent exists,
        	// relink the parent and node and transfer node's subtree.
        	if (left(parentNode) != null && left(parentNode).equals(node)) {
        		relink(parentNode, (BSTNode<Entry<K, V>>) right(node), true);
        		relink(node, parentNode, false);
        	}
        	else {
        		relink(parentNode, (BSTNode<Entry<K, V>>) left(node), false);
        		relink(node, parentNode, true);
        	}
        }
        ...
        public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
        	...
        	BSTNode<Entry<K, V>> node = (BSTNode<Entry<K, V>>) x;
        	BSTNode<Entry<K, V>> parentNode = (BSTNode<Entry<K, V>>) parent(x);
        	BSTNode<Entry<K, V>> grandparentNode = (BSTNode<Entry<K, V>>) parent(parent(x));
        	//if node and parent are both left children OR both right children then
            // rotate the parent around the grandparent
        	if(left(parentNode) != null && left(parentNode).equals(node) &&
        	   left(grandparentNode) != null && left(grandparentNode).equals(parentNode) ||
        	   right(parentNode) != null && right(parentNode).equals(node) &&
        	   right(grandparentNode) != null && right(grandparentNode).equals(parentNode)) {
     ...
     * @param p Position whose parent to get
     * @return p's parent
     */
    ...
    /**
     * Returns true if p is internal (a non-leaf)
     * @param p Position to test
     * @return true if p is internal (a non-leaf)
     */
    ...
    /**
     * Sets Position p with entry
     * @param p Position to set
     * @param entry Entry to set at p 
     ...
     * @param p Position to fetch left child of
     ...
     * @param p Position to fetch right child of
     ...
     * @param p Position to fetch sibling of
     * @return p's sibling
     */
