 ...
 * worst-case performance for {at link Map#put}, {at link Map#get}, and
 * {at link Map#remove}.
 * 
...
public class RedBlackTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
    ...
    private void resolveRed(Position<Entry<K, V>> p) {
    	...
    	Position<Entry<K, V>> middle = null;
    	if (isRed(parent)) {
    ...
    private void remedyDoubleBlack(Position<Entry<K, V>> p) {
    	...
    	boolean siblingIsLeft = false;
    	
    	if (isBlack(sibling))
    	// CASE 1: trinode restructuring
    		// If both siblings' children are red, temp is the opposite child 
    		// of sibling that sibling is of parent
    		if (isRed(left(sibling)) && isRed(right(sibling))) {
    			if (sibling.equals(left(parent)))
    				siblingIsLeft = true;
    			else
    				siblingIsLeft = false; // Parent is right child of grandparent
    			if (siblingIsLeft)
    				temp = left(sibling);
    			else
    				temp = right(sibling);
    		}
    		// else if there is just one red, make that red child temp
    		else if (isRed(left(sibling)) || isRed(right(sibling))) {
    	    	if (isRed(left(sibling))) 
    	    		temp = left(sibling);
    	    	else
    	    		temp = right(sibling);
    	    	middle = restructure(temp);
    	    	if (isRed(parent))
    	    		makeRed(middle);
    	    	else
    	    		makeBlack(middle);
