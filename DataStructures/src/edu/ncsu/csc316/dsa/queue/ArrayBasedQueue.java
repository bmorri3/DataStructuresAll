...
public class ArrayBasedQueue<E> extends AbstractQueue<E> {
    ...
    private void ensureCapacity(int minCapacity) {
        // Old capacity is length of the array
    	int oldCapacity = data.length;
    	// If the old capacity is less than the required capacity, create a new array with needed capacity
        if (minCapacity > oldCapacity) {
            ...
            // Copy starting at the front
            int currentNode = front;
            
            for(int i = 0; i < size; i++) {
            	newData[i] = data[(currentNode + data.length) % data.length];
            	currentNode++;
            }
            // Reset front and rear when array capacity is resized
            front = 0;
            rear = data.length - 1;
    ...
    public void enqueue(E element) {
    	...
    	// Using modulo to calculate the new index. If front + size is greater than data.length
    	// wrap around. Adapted from Data Structures and Algorithms in Java, p. 224
    ...
    public E dequeue() {
    	...
    	// If the array is empty, reset rear to index 0.
    	if(isEmpty())
    		rear = 0;
